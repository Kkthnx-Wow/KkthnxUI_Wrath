...
if not HealComm.unitToPet then
	...
	for i = 1, MAX_PARTY_MEMBERS do
		HealComm.unitToPet["party" .. i] = "partypet" .. i
	end
	for i = 1, MAX_RAID_MEMBERS do
		HealComm.unitToPet["raid" .. i] = "raidpet" .. i
	end
end
	...
	if not HealComm.protectedMap then
		HealComm.protectedMap = setmetatable({}, {
			__index = function(tbl, key)
				return HealComm.guidToUnit[key]
			end,
			__newindex = function()
...
if playerClass == "DRUID" then
	LoadClassData = function()
		...
		AuraHandler = function(unit, guid)
			hotTotals[guid] = 0
			if unitHasAura(unit, Rejuvenation) then
				hotTotals[guid] = hotTotals[guid] + 1
			end
			if unitHasAura(unit, Lifebloom) then
				hotTotals[guid] = hotTotals[guid] + 1
			end
			if unitHasAura(unit, WildGrowth) then
				hotTotals[guid] = hotTotals[guid] + 1
			end
		...
		CalculateHotHealing = function(guid, spellID)
			...
			elseif spellName == Regrowth then
				...
				if talentData[NaturesSplendor].current >= 1 then
					totalTicks = totalTicks + 2
				end
				if equippedSetCache["Nordrassil"] >= 2 then
					totalTicks = totalTicks + 2
				end
...
if playerClass == "PALADIN" then
	LoadClassData = function()
		...
		CalculateHealing = function(guid, spellID, unit)
			...
			if equippedSetCache["Lightbringer"] >= 4 and spellName == FlashofLight then
	...
	LoadClassData = function()
		...
		CalculateHealing = function(guid, spellID)
			...
			if spellName == GreaterHeal then
				if equippedSetCache["Absolution"] >= 4 then
					healModifier = healModifier * 1.05
				end
...
if playerClass == "HUNTER" then
	LoadClassData = function()
		...
		CalculateHotHealing = function(guid, spellID)
			...
			if equippedSetCache["Giantstalker"] >= 3 then
				amount = amount * 1.1
			end
...
HealComm.healingStackMods = HealComm.healingStackMods or {
		-- Mortal Wound
		[25646] = function(stacks)
			return 1 - stacks * 0.10
		end,
		[28467] = function(stacks)
			return 1 - stacks * 0.10
		end,
		[30641] = function(stacks)
			return 1 - stacks * 0.05
		end,
		[31464] = function(stacks)
			return 1 - stacks * 0.10
		end,
		[36814] = function(stacks)
			return 1 - stacks * 0.10
		end,
		[38770] = function(stacks)
			return 1 - stacks * 0.05
		end,
		-- Dark Touched
		[45347] = function(stacks)
			return 1 - stacks * 0.05
		end,
		-- Nether Portal - Dominance
		[30423] = function(stacks)
			return 1 - stacks * 0.01
		end,
		-- Focused Will
		[45242] = function(stacks)
			return 1 + stacks * 0.10
		end,
	}

if isTBC or isWrath then
	HealComm.healingStackMods[13218] = function(stacks)
		return 1 - stacks * 0.10
	end -- Wound Poison (Rank 1)
	HealComm.healingStackMods[13222] = function(stacks)
		return 1 - stacks * 0.10
	end -- Wound Poison (Rank 2)
	HealComm.healingStackMods[13223] = function(stacks)
		return 1 - stacks * 0.10
	end -- Wound Poison (Rank 3)
	HealComm.healingStackMods[13224] = function(stacks)
		return 1 - stacks * 0.10
	end -- Wound Poison (Rank 4)
	HealComm.healingStackMods[27189] = function(stacks)
		return 1 - stacks * 0.10
	end -- Wound Poison (Rank 5)
end
...
local function parseDirectHeal(casterGUID, spellID, amount, castTime, ...)
	...
	if not unit or not spellName or not amount or select("#", ...) == 0 then
		return
	end
...
local function parseChannelHeal(casterGUID, spellID, amount, totalTicks, ...)
	local spellName = GetSpellInfo(spellID)
	local unit = guidToUnit[casterGUID]
	if not unit or not spellName or not totalTicks or not amount or select("#", ...) == 0 then
		return
	end
...
local function parseHotHeal(casterGUID, wasUpdated, spellID, tickAmount, totalTicks, tickInterval, ...)
	...
	if not tickAmount or not spellName or select("#", ...) == 0 then
		return
	end
	...
	if not stack or stack == 0 or duration == 0 or endTime == 0 then
		return
	end
...
local function parseHotBomb(casterGUID, wasUpdated, spellID, amount, ...)
	...
	if not amount or not spellName or select("#", ...) == 0 then
		return
	end
	...
	if not hotPending or not hotPending.bitType then
		return
	end
...
local function parseHealEnd(casterGUID, pending, checkField, spellID, interrupted, ...)
	local spellName = GetSpellInfo(spellID)
	if not spellName or not casterGUID then
		return
	end
	...
	if not pending or not pending.bitType then
		return
	end
...
local function parseHealDelayed(casterGUID, startTimeRelative, endTimeRelative, spellID)
	...
	if pending.endTime == endTime and pending.startTime == startTime then
		return
	end
...
function HealComm:CHAT_MSG_ADDON(prefix, message, channel, sender)
	if prefix ~= COMM_PREFIX or channel ~= distribution then
		return
	end
	...
	if not commType or not spellID or not casterGUID or casterGUID == playerGUID then
		return
	end
...
local function setCastData(priority, name, guid)
	if not guid or not lastSentID then
		return
	end
	if guidPriorities[lastSentID] and guidPriorities[lastSentID] >= priority then
		return
	end
	...
	if castTarget and castTarget == name then
		priority = 99
	end
...
function HealComm:UNIT_SPELLCAST_START(unit, cast, spellID)
	...
	if not spellData[spellName] or UnitIsCharmed("player") or not UnitPlayerControlled("player") then
		return
	end
	...
	if not targets then
		return
	end -- only here until I compress/decompress npcs
...
function HealComm:UNIT_SPELLCAST_STOP(unit, castGUID, spellID)
	local spellName = GetSpellInfo(spellID)
	if unit ~= "player" or not spellData[spellName] or spellData[spellName]._isChanneled then
		return
	end
...
function HealComm:UNIT_SPELLCAST_INTERRUPTED(unit, castGUID, spellID)
	local spellName = GetSpellInfo(spellID)
	if unit ~= "player" or not spellData[spellName] then
		return
	end
...
function HealComm:UNIT_SPELLCAST_DELAYED(unit, castGUID, spellID)
	...
	if unit ~= "player" or not pendingHeals[casterGUID] or not pendingHeals[casterGUID][spellName] then
		return
	end
